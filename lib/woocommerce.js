...
export class WooCommerceError extends Error {
  constructor(message, status, body, code = 'WOOCOMMERCE_ERROR', details = {}) {
    super(message);
...
function safeUrlForLogs(url) {
  ...
  safeUrl.searchParams.delete('consumer_key');
  safeUrl.searchParams.delete('consumer_secret');
  return safeUrl.toString();
}

function addWooQueryAuth(url) {
  const env = getWooEnv();
...
function buildWooUrl(path, mode = 'pretty') {
  const env = getWooEnv();
  const route = `/wc/v3${path}`;
  let url;
  try {
    if (mode === 'query') {
      url = new URL('/', env.url);
      ...
      url = new URL(`/wp-json${route}`, env.url);
    }
  } catch (error) {
...
export async function wooFetch(path, options = {}) {
  ...
  let body = parseMaybeJson(text);

  if (response.status === 404) {
    const fallbackUrl = buildWooUrl(path, 'query');
    ...
    if (!response.ok) {
      ...
      throw new WooCommerceError(
        ...
        'WOOCOMMERCE_ERROR',
        {
          request_url: safeUrlForLogs(fallbackUrl),
          first_request_url: safeUrlForLogs(url),
        }
      );
    }
  ...
  if (!response.ok) {
    ...
    throw new WooCommerceError(
      ...
      body,
      'WOOCOMMERCE_ERROR',
      { request_url: safeUrlForLogs(url) }
    );
  }
