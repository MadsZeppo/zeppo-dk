export const MENU = [
  ...
  if (spoken.length < 5 || /^\d+$/.test(spoken)) {
  return null;
  }
  ...
  for (const item of NORMALIZED_MENU) {
  const isExactMatch =
  spoken === item.normalizedCanonicalName ||
  item.normalizedAliases.includes(spoken);
