...
- låsetype = kun hvis kunden selv nævner Ruko, Assa, Dorma, EVVA eller anden låsetype. Ellers "Ikke oplyst".
...
export function buildLaasesmedSms(info) {
  ...
  linjer.push(`Adresse: ${adresse || 'Ikke oplyst'}`);

  if (isKnown(info.etage_dør)) linjer.push(`Etage/dør: ${safe(info.etage_dør)}`);
  ...
  if (isKnown(info.person_inde)) linjer.push(`Person/dyr inde: ${safe(info.person_inde)}`);
  if (isKnown(info.id_klar)) linjer.push(`ID: ${safe(info.id_klar)}`);
